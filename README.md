...
> ⚙️ **一切都可以在侧边栏 WebUI 里配置（更方便）**
>
> 插件页面（KiraAI 侧边栏 →「全能浏览器」）现在是一个**完整控制台**：
>
> - **全部配置项**都能在那儿改 —— 界面是照着插件自己的 `schema.json` 生成的，
>   插件加了新选项，界面**自动出现**，不用改前端
> - **改完立刻生效，不用重启 KiraAI**（改动写进 `<插件数据目录>/webui_config.json`，
>   叠在框架配置之上；面板一保存就重跑一次 `_apply_config`）
> - 同一页还能看连接状态、当前后端、命令计数，以及生成 / 复制接入令牌
>
> 下面这份表仍然有效（它就是面板里那些项的说明），只是**不用再去翻配置文件**了。
...
| `headless_enabled` | 开 | 单独关掉「插件自带的无头浏览器」这条路 |
...
<details>
<summary><b>2.1.x</b> — 58 个版本　·　最新的一系列：双后端重构、安全加固、以及大量审查修复</summary>
...
上一版修截图时顺带发现：`_call()` 一直是"失败就试下一个后端"。用户直接把规矩定死：

> 除了一开始使用用户浏览器不了，才会提示并直接降级使用无头，其他时候尤其是操作中，
> 肯定不能回退或者直接切换。……所有这种操作中肯定不能自动呀，那不就中途断掉出错了吗？

对。中途换后端 = **换了操作对象**，前面那步的结果也就断了；读/截图更糟：拿到的是
**另一套浏览器**的画面却报"成功"（上一版那个"空白截图 + ✅"就是这么来的）。

#### 现在的规矩

1. **首选可用、但这次调用失败 → 如实失败**，绝不试下一个；回话里附一句"怎么显式换"。
2. 唯一例外：首选**从一开始就不可用**（没连上 / 没启动）→ 才用下一个，并且**在结果里明说**
   （"两套独立的浏览器"）。第一次说全，之后每次只留一行 —— 不制造新噪音。
3. **新增工具 `browser_backend`**：`status` 看当前用哪个；`use` 显式切换
   （`extension` / `headless` / `auto`）。用户说一句"用无头"时，bot 就有落点了。
4. **用户把「后端策略」固定成某一类时，这个工具必须拒绝** —— 那是用户的决定
   （用户明确要求：配置选了某一类，就不许切换）。
5. 整页 / 元素截图：扩展做不到就如实说"想整页请显式切到无头"，**不再**把无头浏览器的
   画面当成用户的整页截图返回。
6. 切换是**运行时**的（只在内存里），插件重载后回到配置值 —— 不让 bot 一次调用就
   永久改掉用户的设置。

#### 验证

`failover_notice` 重写为 7 条，全部**真调** `_call` / 真调 `browser_backend`：

- 首选失败时**无头一次都没被调用**（这正是"不许中途换"的硬判据）；
- 首选从开始就不可用才降级，且结果里必须出现"两套浏览器"的说明；
- 用户固定策略时，切换工具明确拒绝（`_backend_override` 保持为空）；
- `auto` 下显式切换真的生效（候选列表变成无头、并把没启动的无头拉起来）；
- 正常路径**零提示**（不许每轮都刷）；
- **反向自检**：把 `allow_failover` 翻成 `True`（模拟"失败即换"的老行为）→ 判据必须报红。

**回归 525/525。**

### v2.1.86（2026-09-22）

**截图：窗口最小化不再莫名其妙失败；而且失败**不再被静默换成另一个浏览器**。**

#### 用户报的现象（两条，第二条是顺着代码查出来的）

1. 把浏览器最小化到任务栏之后截图失败：
   `[screenshot] 用户浏览器（Edge · 扩展 v1.5.2）失败：Failed to capture tab: image readback failed`
2. 更糟的是：这行只是 WARNING —— `_call()` 会继续试下一个候选后端，而无头后端截的是
   **它自己那个浏览器**。于是 **扩展失败 → 换成无头 → 照样返回「✅ 截图已保存」**：
   模型拿到一张空白图，还以为那是用户的浏览器（正是仓库一直在防的"假成功"）。

#### 根因

- **最小化那条**：窗口不可见（最小化 / 被完全遮挡）时 Chromium **读不到帧**，
  `captureVisibleTab` 必然失败。这不是我们的 bug，但**处理**得很差：不判断窗口状态、
  不重试，还把浏览器原文（`readback failed`）丢给用户 —— 看不懂，也没法照做。
- **静默回退那条**：返回里那句 `（来源：…）` 只是**标注**，不是**事件**。模型不会因为
  一个小括号就意识到"我原本要操作的那套浏览器失败了、这是另一套的画面"——
  截图尤其危险，它会把图当成用户看到的东西。

#### 改法

- **扩展侧**：截图前先看窗口状态；只为"拿不到画面"这类错误动窗口 ——
  恢复 → 等一帧（最多 3 次）→ 重截 → **把窗口还原成最小化**（借东西要还）。
  别的错误（没权限、内部页…）一律**不碰用户窗口**：不许为了重试把人家的窗口弹出来。
  彻底失败时给能照做的话（"把窗口恢复出来再重试"），不再透传浏览器原文。
- **配置项「截图时临时恢复窗口」**（默认开）：会短暂弹窗这件事，用户有权关掉。
- **插件侧**：`_call()` 新增 `no_fallback` —— 换后端就换语义的操作（截用户浏览器
  可视区域）**宁可直接失败**；允许切换的场合（整页 / 元素截图，扩展本来就做不到），
  返回文本以「🔁 后端已切换」开头，写清原因和"那是另一个浏览器、不是用户浏览器当前画面"。
- 扩展版本 **1.5.3**（改了 background.js，按 v2.1.83 立的守卫必须升 + 刷戳）。
...
| 检查 | 拿**改动前**的代码跑同一套判据 |
|---|---|
| `screenshot_restore`（行为探针，7 条） | ❌ 抽不到自愈实现（自报，不静默失效） |
| `failover_notice`（真跑 `_call`，6 条） | ❌ **J2：无头被调用 1 次、回话是 `✅ 截图已保存 … （来源：无头浏览器）`** —— 静默假成功当场复现 |

行为探针里最要紧的一条是"**用完把窗口还原成最小化**"—— 借了用户的窗口一瞬，必须还。

### v2.1.85（2026-09-22）

**修「查看更新步骤」一闪一闪 —— 它其实从来没有"收放"过。**

#### 用户看到的现象

面板提示"扩展建议更新"，点「查看更新步骤」，那个块**一闪一闪**，
再点也没有反应（不会收起）。

#### 根因（两条叠在一起）

1. **提示块每 3 秒被整个重建一次**：面板 `setInterval(refresh, 3000)` →
   `renderStatus` → `renderExtNotice`，而它**无条件**重写 `#extNotice` 的
   innerHTML —— 连里面的按钮一起换掉，用户展开的东西自然被打回初始态。
2. **步骤块每 3 秒被重新拉一次**：`_extStepsShown` 为真时，它每次都会再调一次
   `loadExtSteps()`，而那个函数先把内容清成"正在读取安装步骤…"再去请求
   `/extension` ⇒ **清空 → 填满 → 清空 → 填满**，正是看到的一闪一闪。

顺带发现：**"收"这个动作根本不存在** —— 按钮只是在两种加载之间来回，
点多少次都是重拉，从来没有收起。

#### 修法

- `renderExtNotice` 加**内容签名**（state + 两个版本号）：内容没变就**一个字节都不动**；
- 步骤内容**只拉一次**（`_extStepsLoaded`），之后点击只做收放；
- 按钮变成真正的开关：展开 →「收起更新步骤」，收起 →「查看更新步骤」，带 `aria-expanded`；
- 收放用 `.fold{grid-template-rows:0fr ↔ 1fr}` 过渡（**内容高度未知**也能平滑；
  纯 CSS 计时、不依赖 `transitionend` ⇒ 开了"减少动效"瞬时完成，不会卡在半开）；
  收起时连 `visibility` 一起关掉，里面的链接不会被 Tab 键选中；
- 扩展更新好之后照旧收起并清空（下次需要时重新拉最新步骤）。

#### 验证：先复现，再修

`regression/js/panel_smoke.mjs` 新增 7 条行为断言，其中一条用 **MutationObserver**
直接盯"展开的步骤块在轮询期间被改动过几次"（比事后看内容对不对灵敏 ——
旧代码里那一闪是瞬时的，轮询间隙根本抓不到）：

| | `/extension` 调用 | 步骤块 DOM 变更 | 结果 |
|---|---|---|---|
| 修复前 | 1 → **4 次** | **6 次** | 6 条报红 |
| 修复后 | **1 次** | **0 次** | 全绿 |

另加静态守卫 `panel_ui.S25`（把"签名守卫 / 只拉一次 / fold 结构 / 开关文案"钉住，
含反自检）。

顺带修掉一处**探针没跟上重构**：`status_race.mjs` 的沙箱缺 `setExtSteps`
与 `classList` / `setAttribute` 桩 ⇒ 整段 `refresh()` 抛错、返回空结果
（第 4 次踩这个坑了，所以这次把桩按真 DOM 补齐，而不是让产品代码绕开）。
...
- `browser-bridge/icons/` 三张（16 / 48 / 128）换成看板娘立绘：
  青黑长发少女，身周环绕**发光的浏览器窗口** —— "AI 替你开窗"这件事的具象化。
- 人物设定：**现代少女**（针织开衫 + 衬衫 + 百褶裙），不是法师 ——
  没有法杖、没有法阵、没有施法动作，发光的是窗口本身，不是她。
  配色跟着插件走：奶油 `#FEF6EA` / 青绿 `#DBF0E8` / 深墨 `#4A585D`。
- **三个尺寸各自出图，不是一张缩三次**：16px 用**面部特写裁切**
  （全身缩到 16px 必糊成一团色块），48 / 128 用完整构图。
- 圆角 + 透明通道（跟旧图标一致），`pngquant` 量化 + `oxipng` 无损优化：
  128px **37.4 KB → 11.5 KB**、48px **5.9 KB → 2.9 KB**、16px **895 B → 782 B**，
  三张合计约 **15 KB**（塞进扩展包没有负担）。
- 扩展版本同步升到 **1.5.2** —— 换图标同样是"扩展内容变了"，按 v2.1.83 立的
  守卫必须升版本，戳已刷新 ⇒ 跑 1.5.1 的用户会在面板看到"扩展可以更新"。

#### 新增守卫

`regression/checks/ext_icons.py`：manifest 声明与文件是否对得上、
实际像素是否等于声明尺寸、是不是真 PNG、**单张与合计的体积预算**
（128 ≤ 40 KB / 48 ≤ 12 KB / 16 ≤ 4 KB / 合计 ≤ 60 KB），
并带**反向自检**（往临时副本塞一张超预算的假图标，检查必须报红）。
防的是以后有人把一张几百 KB 的原画直接丢进 `icons/` —— 扩展包白胖十几倍，
而用户装扩展时是要把这些字节下载一遍的。

### v2.1.83（2026-09-22）

**扩展版本升到 1.5.1 —— 上一版漏了这一步，等于白改。**

#### 事情的经过（用户问出来的）

用户看面板：插件已经是最新的 v2.1.82、扩展显示 `v1.5.0 ✓`，
**却没有收到"扩展可以更新"的提示**。问得很对，因为答案就是：

上一版（v2.1.82）插件侧加了"空闲先探测再判死"、扩展侧加了"半开连接自愈"——
**扩展代码改了四个文件（background.js / shared.js / popup.js / protocol.js），
但 `browser-bridge/manifest.json` 里的 version 还是 1.5.0** ✗

面板比的是"你装的版本"和"插件自带的版本"，两边都是 1.5.0 ⇒ 判定"已是最新"⇒
一个字都不提示 ⇒ **用户永远停在旧扩展上，新加的自愈能力一个也吃不到**。
而这套提示本来就是为了防这个的 —— 只能说我上一版漏了一步，坑自己先踩了。

#### 修法

1. `browser-bridge/manifest.json` → **1.5.1**（这次真的把代码和版本一起升了）；
2. **加一条守卫，让这件事以后不可能再发生**：
   `browser-bridge/.version-stamp.json` 记下"上次升版本时扩展目录的**内容指纹**+版本号"，
   回归里逐条对账：

   * 指纹变了、版本号没变 → **报红**（就是上面那个坑）
   * 版本号变了、指纹没刷新 → **报红**（忘了跑刷新脚本）

   改完扩展代码的正确姿势（现在写在 `regression/ext_version_stamp.py` 的文件头里）：

   ```
   # 1. 把 browser-bridge/manifest.json 的 version 往上加一级
   # 2. 刷新版本戳
   python3 regression/ext_version_stamp.py
   ```

   守卫自带**反向自检**（把扩展代码改一个字节，判据必须能发现 ——
   证明指纹真的覆盖内容，不是摆着好看的）。

#### 回归

`ext_update` 检查新增 V8（含反自检）；反向验证：把版本号改回 1.5.0、
戳不动 → 检查立刻报红并说清该怎么办。套件 **495/495**。

### v2.1.82（2026-09-20）

**面板会主动告诉你"浏览器里的扩展该更新了"。**

#### 为什么需要

扩展是**随插件一起打包**的（就在插件目录的 `browser-bridge/` 里），
但用户浏览器里装的那份不会跟着插件自动更新。插件加了新能力、
旧扩展吃不到时，界面上只会表现成"某个功能不好用"，很难联想到是扩展旧了。

最典型的就是上一版：插件侧把桥接改成"空闲先探测再判死"，扩展侧加了
半开连接自愈 —— **没更新的扩展就享受不到自愈**，而用户完全不会知道。

#### 做法（**判断只在后端做一次**，"未来都可以检测"）

* 后端拿两处**真源**比版本：插件自带的 `browser-bridge/manifest.json`，
  和扩展握手时上报的 `chrome.runtime.getManifest().version`；
* `/status` 多出一块 `extension`：`state` / `needs_update` /
  `bundled_version` / `connected_version` / `protocol_ok`；
* 面板只按 `state` 取文案、把版本号显示出来 —— **前端没有任何版本号字面量**。

⇒ 以后插件把扩展升到 1.6.0、用户那边还是 1.5.0 时，提示**自动出现**，
前端一行都不用改。（有一条检查专门验证这件事：拿假 manifest 模拟"未来版本"，
状态必须变成"可更新"。）
...
| 情况 | 面板表现 |
|---|---|
| 扩展比插件自带的旧 | 提示更新（版本号都列出来） |
| 协议版本不一致 | 提示更新（版本号可能一样，但能力对不上） |
| 连上了但没上报版本（很旧的扩展 / 握手没成） | 提示"重新加载或按步骤更新" |
| 扩展比插件自带的还新（用户自己换了新版） | **不提示**（别去催他"更新"） |
| 没连上 | 不提示（本来就有未连接提示，不重复喊） |

提示长这样（就在「连接状态」卡片里，扩展版本那一格也会显示
`v1.4.0（插件自带 v1.5.0）`）：

> ⚠ **扩展建议更新** —— 扩展可以更新了：你现在用的是 v1.4.0，插件自带的已经是
> v1.5.0。旧版扩展可能缺一些修复与新能力（例如连接空闲时的自愈），
> 更新后功能才完整。 ［查看更新步骤］

「查看更新步骤」复用现成的 `/extension` 接口（扩展的绝对路径 + 该浏览器的
具体步骤），不另写一份文案 —— 步骤改了这里自动跟上。

#### 顺手修的

`status_race.mjs` 探针跟着重构补齐了桩（`renderExtNotice`）——
第三次遇到"探针没跟上重构、看着像守卫失效"了，这次的探针会自己说明
抽不到哪个函数。

#### 回归

新增检查 `ext_update`（9 条）：版本比较（含 `1.10 > 1.9` 这种字符串陷阱、
预发布、垃圾串 → 不瞎判）、状态机（该提示的提示、不该提示的闭嘴）、
"插件升版本 → 提示自动出现"、**前后端状态名必须一致**（后端
`EXT_NOTICE_STATES` ↔ 前端文案键，含反向自检）、前端不许写死版本号、
接线完整、以及真跑一遍 `api_status` 看字段齐不齐。
面板真 DOM 探针加了 4 条：提示出现（含两个版本号）、点开能看到步骤、
更新后提示自动消失。套件 **488/488**。
...
这**不是故障**，但比故障更烦人：刷屏，而且看不出要不要管。

#### 为什么会有这条规则

它本来是防**半开连接**的：扩展的 MV3 Service Worker 被浏览器回收之后，
WebSocket 可能停在"服务端还认为连着、对面其实收不到"的状态 ——
此时 `connected` 一直是 True、面板显示"已连接"，而每个工具调用都卡到超时。
读超时（心跳 25s × 2 = 50s）就是用来兜这个的。

#### 那为什么变成"假断开"

因为**"Service Worker 睡着了"和"真的死了"从服务端看一模一样**：
回收后扩展会静默几十秒，等保活闹钟（30s）把它唤醒，它自己就回来了。
于是 50 秒一到就被判死 → 1 秒后重连 → 再静默 → 再判死 …… 循环刷屏。

#### 现在怎么做（两侧一起改）

**服务端改成两段式**：

* 静默超过上限（现在 = 心跳 × 3 = **75 秒**，盖得住一次"回收 → 唤醒"）
  → 只**发一个探测包**，再用 **6 秒**的短窗口等回应；
* 窗口内**回话了**（哪怕只是 pong）→ 虚惊一场：**不断开、不换 session、不刷日志**；
* 窗口内还是没动静 → 才判死；而且同一个 5 分钟窗口里**只告警一次**
  （其余只记数，日志不再被刷屏）。

**扩展侧自己会修**：保活闹钟每次唤醒时，

* 如果"看着 OPEN、但已经 60 秒没收到服务端任何一帧"（服务端每 25 秒 ping 一次，
  所以这一定不正常）→ **自己换一条新连接**（比等服务端判死干净得多）；
* 否则发一条 ping，让服务端看见"扩展这边醒着"—— 服务端收到任何一帧都会重置
  空闲计时，于是"对面明明活着却被判死"这件事从根上少了一大截。

**顺带**：`/status` 里多了 `idle_probes` / `idle_disconnects` / `last_inbound_ago`，
面板自检那一行会显示"桥接空闲探活 N 次 / 判死 M 次" —— 以后再看到连接抖动，
一眼就能判断是正常抖动还是真有问题。

**"换一条新连接"那句话本身也不刷屏**：扩展侧这个动作在链路真半开时
每分钟都会发生一次，一条条打进控制台等于换了个地方刷屏。所以它同样降噪：
同一个 5 分钟窗口内只提示一次，其余只累加计数；计数**落盘**（Service Worker
被回收也不丢），并显示在扩展弹窗上（"半开连接自愈：扩展自己换过 N 次连接"）——
正常应该长期是 0，数字持续增长才说明链路真有问题。

#### 回归

真 WebSocket 端到端新增三条：**E9** 扩展静默 1.2 秒后回来 →
不许断开、session 不许换；**E10** 探测窗口内仍无回应 → 才判死（且计数 +1）；
**E11** 探测期间**迟到的命令结果不许丢**（探测会取消一次 recv，丢消息的话
调用方会一直干等到超时）。
另加一个检查 `bridge_idle`：判死日志降噪（同窗口只喊一次）、容错窗口
必须 ≥ 3 个心跳、扩展侧半开连接自愈的接线。

实测对照（同一场景，静默 1.2 秒后自己回来）：

| | 日志 | 连接 |
|---|---|---|
| 改前 | `WARN 判定连接已失效，主动断开` | 断了、session 换了 |
| 改后 | `INFO 1s 没收到扩展数据，发探测包确认` | **还在，session 不变** |

套件 **474/474**。

### v2.1.80（2026-09-20）

**面板的字段渲染整体对了一遍：以前"该是下拉的成了填写框、该有值的却是空框"。**

#### ① 令牌旁边那只眼睛，点了没反应

`#eye` **根本没绑点击事件** ✗ —— 图标画出来了，点了什么也不发生
（所以令牌只能一直遮着，复制按钮能用但看不见）。

修：绑上切换（点一下显示明文，再点遮回去）。

#### ② 该是下拉的字段变成了填写框

面板只认 `model_select`，于是 schema 里带 `options` 的**枚举字段**
（后端策略 / 浏览器来源 / 无头 profile 模式 / 默认等待策略）全被渲染成
自由文本框 ✗ —— 用户手打 `Auto`、`chrome ` 这种拼错的值会被原样存下去。
而框架自己的设置页里它们本来就是下拉（`hasOptions(field)` → 选择器），
两边行为不一致。

修：**带 `options` 就渲染成下拉**，并在下拉里把默认项标成 `auto（默认）`。

顺带把**字段类型判断与框架对齐**（框架的 `configFieldTypes.ts` 认
`integer/float/number`、`switch/boolean/bool`、`list`、`multi_select`）：

* `float` 以前不认 ✗ —— 「命令超时」「跟随框架时的比例」被当成文本框渲染
* 于是 `list` 类的判断也统一了（列表字段不再依赖"值恰好是数组"）

#### ③ 有默认值的字段却是空框

面板上「命令超时 / 页面加载超时 / 交互操作超时 / 浏览器来源 / profile 模式 /
自定义 profile 目录 / 默认视口大小」全是**空框**，用户以为"没设置"。

真根因：这些键（`timeout` / `op_timeout` / `command_timeout` /
`browser_channel` / `headless_profile_mode` …）**根本不在插件实例上** ——
它们是**传给后端**的，由 `backends/headless_backend.py` 自己
`cfg.get(键, 默认值)` 读。而面板回显用的快照只做 `getattr(self, 键)`，
拿不到就给了 `None` → 空框。

修：快照按 `实例属性 → 框架配置 → 面板覆盖 → schema 默认值` 依次回退
（这些键的**代码默认值与 schema 默认值是一致的**，有检查盯着）。
另外输入框空着时，用占位符把默认值显示出来。

#### ④ 帮助文本里写清默认值（用户明确要求）

54 个配置项全部补齐：**中文提示 + 英文提示**都写明默认值是多少；
原来没写说明的 13 个字段（启用插件 / 命令超时 / 视口大小 / 截图保留数 …）
补上了说明。中文用「默认：X。」，布尔写「开 / 关」，列表把每一项列出来。

#### ⑤ 保存只提交**改动过**的字段

快照能回退默认值之后，如果保存还是把整张表都存下去，就等于把"当前默认值"
**钉成覆盖值** —— 以后插件升级改了默认值，这些用户再也跟不上。

现在只提交用户改过的那几项（保存提示也会写明"已保存 N 项"），
实测一次只改一个字段时，落盘的就是那一个键。

#### 回归

新增 5 条行为断言（枚举是下拉 / 下拉标默认 / 数值框认 float /
空值显示默认 / **点眼睛能看到明文**）+ 4 条静态守卫
（S21 提示里必须写默认值 · S22 枚举必须渲染成下拉且默认项在选项内 ·
S23 字段类型判断与框架对齐 · S24 快照覆盖所有字段，不许有空框）。
**已反向验证**：拿修复前的代码跑，这 9 条全红。套件 **456/456**。

### v2.1.79（2026-09-20）

**侧边栏面板的 4 个问题，这次全部在真浏览器里复现、修好、再验证。**
（前几轮都是"看代码推断"，所以一直没修到点上 —— 这次先搭了一个把**真插件**
跑在仿 KiraAI 服务器上的验证台，用真浏览器打开面板，逐条对着看。）

#### ① 令牌永远显示"读取失败" —— 是 `id` 对不上，不是什么网络问题

`app.js` 里读写 `$("tokBox")`，而 `index.html` 里**根本没有这个 id** ✗
→ 取到 `null` → `null.dataset` 抛类型错误 → 被自己的 `catch` 吞掉 →
界面只剩一句笼统的"读取失败"。看着像网络/权限问题，其实就是少写了一个属性。

修：HTML 补上 `id="tokBox"`；同时令牌的存取改成走一个小助手，
拿不到容器也**绝不会**变成"读取失败"。失败时把**原因**写出来
（HTTP 几几 / 后端说了什么），不再只给一句"读取失败"。

> 顺带纠正上一版日志里的一个错误结论：当时把"读取失败"归因于
> "POST 不带 body 会被 FastAPI 拒成 422"。实测（FastAPI 0.141）不带 body
> 照样返回 200 —— 那条**站不住**，真正的根因就是上面这个 `id`。

#### ② 保存之后，"保存并立刻生效"的弹窗不消失

`saveConfig()` 的 `finally` 里**无条件**又把保存条打开了一遍 ✗ ——
于是成功保存之后，它还挂在那里、还写着"有未保存的改动"，
让人以为没存上。

修：成功就收起（并清掉"已改动"标记），**只有失败才留着**；
文案也改成"有 N 项未保存的改动"，改了几项一目了然。

#### ③ 换了 VLM 模型，下拉框没有任何反应

两处都漏了 `<select>` ✗：改动的监听只绑了 `input/textarea`（所以下拉改了
不点亮保存条），而 `collect()` 也只收 `input/textarea`
（所以**就算点了保存，那个值也不会被提交**）。

修：事件改成在配置区上**统一委托**，`input / textarea / select` 一起管；
提交时也把下拉的值带上。改动即时在字段上标出来。

#### ④ 开屏动画"根本不出现"

两个原因叠在一起：

* "减少动效"偏好（Windows：设置 → 辅助功能 → 视觉效果 → 动画效果 关掉就是它）
  那条媒体查询里写了 `.boot{display:none!important}` —— 整层直接不显示 ✗
* 以前还会按 `sessionStorage` 里的时间戳"跳播"：90 秒内重开面板就不播了，
  而"重开面板"恰恰是最常做的事 ✗

修（照 Z 插件那套已经跑通的做法）：减少动效下**照播**、只是更短更干脆
（通配关动画时把 `.boot` 子树排除掉，别把收尾动画一起灭了）；
**每次打开页面都播**，冷却只用在"切走又切回来"这一种情形；
再加一道 JS 硬兜底 —— 万一动画因为任何原因没跑，那一层也一定会被收掉。

#### 面板自检

「连接状态」里多了一行自检：载入动画播没播（以及为什么结束）、
系统是不是开了减少动效、最近一次接口错误。以后有问题截一张图就够。

#### 回归套件

新增 `panel_ui` 检查：用 jsdom 加载**真的**面板，跑完整流程
（开屏动画 → 令牌 → 改下拉 → 保存 → 提示条自己消失），
外加 5 条静态守卫（id 必须存在 / 三种控件都要管 / 成功必收起 /
开屏动画三条保险 / 提示文案不许出现加强符号）。
现在 **446 项全绿**，并且已反向验证：拿修复前的代码跑，这 12 条会红。
...
之前我自己拍脑袋写的版本有毛病（页面会先漏出来、收尾靠 JS 掐时间）。
Z 插件 `web/index.html` 里那套是**已验证可行**的，直接照它的结构来：

| 要点 | 做法 |
|---|---|
| 覆盖层位置 | body **最前面** |
| 关键样式 | **内联在 `<head>`** —— 外链会先画一帧没样式的内容，页面就"漏"出来了 |
| 背景 | **从第一帧就不透明**，只有内容在动 |
| 收尾 | **由 CSS 自己完成**（`animation ... 3.1s forwards`），不依赖 JS 时机 |
| 关掉时 | 紧跟覆盖层的一段内联脚本里 `hidden = true`，**渲染前**就藏掉，一个闪都没有 |
| 跳过 | 点击任意处（`.boot.skip` 走一段短动画抹掉） |
| **重播冷却** | 同一标签页 90 秒内不重播（新配置 `boot_replay_seconds`）—— 否则在页面间来回切会一遍遍放 |

**视觉故意跟它不一样**：Z 插件是图标 3D 翻转；我们走"**墨迹化开 → 星点依次亮起
→ 笔刷斜着抹掉**"（星点呼应图标上那些星星）。

#### 模型下拉：改成插件后端提供 `/models`

Z 插件的做法是**插件自己**开一个 `GET /models`，而不是让面板直连框架接口 ——
这样能：

- 只取 `model_config.llm` 组（视觉模型也得配在 LLM 组里才选得到）
- `id` 用 `provider_id:model_id`（**配置里填的就是这个格式**）
- `name` 用 `模型名 (提供商名)` —— **下拉里显示的是提供商的名字，不是那串 id**

拉不到列表时**优雅退回文本框**并说明可以手填。

#### 守卫

- **S13** 加强：令牌调用必须**同时**是 `POST` **且** `force` 在 query 上
  —— 这就是"旧版能用、重写后不能用"的真正差别（我重写面板时把
  `{ method: "POST" }` 弄丢了 ✗ → 变成 GET → 端点只注册了 POST → 404）
- **S14** 前端资源里不许混进工具输出（就是它把 `style.css` 写坏、害得启动
  动画没样式的）
...
**启动动画不出现的原因找到了（一个把 CSS 写坏的低级失误）+ 令牌 force 绑错位置 + 模型下拉。**

#### 🔴 启动动画为什么没出现

`style.css` 第 200 行**混进了一句工具的输出**：

```
[CONTEXT OFFLOADED] Content (~1523 tokens, ...) saved to: ...
```

我早先一次追加内容太大，工具把它**换成了一句提示文字**，而那句话**被当成
文件内容写进了 CSS** ✗ —— 从那行往后，**整个样式表都废了**，启动动画的样式
自然也没生效。删掉那一行就好了。

> 教训：**追加长内容一定要回读**。这次是靠"用户说没出现 → 去看实际服务出去的
> CSS"才发现的 —— 光看本地文件我可能一直以为是好的。
> 已加守卫 **S14**：前端资源里不许出现这类工具输出。

#### 🔴 令牌 force 绑错了位置（顺带解释了你之前"又连不上"）

```js
api("/token", { method: "POST", body: JSON.stringify({ force: false }) })   // ✗
```

**FastAPI 对简单类型（bool）默认按 query 绑定** —— 塞进 JSON body **根本没绑上** ✗
→ 端点走它自己的默认值 `force=True` → **每次打开面板都会重新生成令牌**，
旧令牌当场作废 → 扩展刚配对好就又连不上 ✗

→ 改成 `POST /token?force=false`（只是读取）/ `?force=true`（点「重新生成」）✓

> 你问"真的需要显示令牌吗"——**显示**只是给手动配对用的；
> 自动配对靠的是页面加载时把令牌推给扩展 ✗ 不依赖这个显示。

#### 模型下拉

`model_select` 之前被当成普通文本框渲染 ✗。现在：

- 从框架的 `/api/providers` + `/api/providers/<id>/models` 拉清单，渲染成**真正的下拉**
- **显示的是提供商的名字，不是 id**（拿不到才退回 id）
- 拉不到时**优雅退回文本框**并说明可以手填 —— 总比给个空下拉、没法选要好

#### 自定义 UA 的说明

写清了**插件已经默认填了一枚合适的**（真实桌面 UA、不含 Headless、
版本跟浏览器主版本对齐、平台按系统推断），留空即可 ✗
只有要伪装成别的浏览器/系统时才填。
...
**面板修 5 个真问题 + 启动动画（可关，默认开）+ 文案去加强符号 + 令牌默认隐藏。**

#### 那 5 个问题的根因

| 现象 | 根因 |
|---|---|
| 版本号不显示 | **`main.py` 里没有 `import json`** ✗ → `json.loads` 抛 `NameError`，被宽泛的 `except` 吞掉 → 返回空串 |
| 配置项 0 项、导航点不动 | **同一个 `NameError`** ✗ —— `_schema_fields()` 也返回空 |
| 令牌显示"读取失败" | 面板用的是 `GET /token`，而端点只有 **`POST /token`** ✗ → 404 |

> 那一行 `import json` 同时解决了前两个 ✗ —— 而它们之所以没被发现，
> 是因为我的 `except` 把异常**吞得干干净净**（正是我一直在说别做的事）。

#### 其余

- **品牌区换成插件图标**（1024px 原图压成 **19KB WebP**，另出一份 4.5KB 的给图标位）
- **令牌默认隐藏**（显示 `••••••••`），点右边的眼睛才看；复制按钮照旧拿真值
- **文案去掉所有 `**加强符号`** —— 它在这套渲染里**不会变成粗体，只会原样显示星号** ✗

#### 启动动画

用**插件图标本身**当主角：图标化开、两道弧线（青绿 + 琥珀）扫过、
几颗星点依次亮起（呼应图标上的星星）、最后整块像被笔刷斜着抹掉。

- **一定盖住页面**：关键 CSS **内联在 `<head>`** —— 放外链的话浏览器会先画
  一帧没样式的内容，再"跳出"动画 ✗（就是"页面先漏出来"那种感觉）
- **可开关，默认开**：`boot_animation`；开关状态记在 `localStorage`，
  **在渲染前一帧就判断** —— 关掉是真的一个闪都没有，而不是"播完再藏"
- 星点用 **SVG 数据 URI**（`clip-path` 在这套渲染里不生效，会变成方块 ✗ 实测过）
- `prefers-reduced-motion` 下不播

#### 守卫跟着学会了三件事

1. **`(x) => ...` 也算参数声明** —— 只认 `x =>` 会把 `(x) => x.trim()` 里的
   `x` 报成未声明（实测误报两个 ✗）
2. C16e 现在扫 **`app.js` + 内联脚本**（JS 拆出去后，它原来扫的是空 ✗；
   而且 `<script>` 的贪婪匹配会把多段内联当成一块 ✗）
3. **"只有前端用"的配置**（启动动画开关）不再被 A1/C1/C2 误判成孤儿 ——
   判据里把 `web/app.js` 也算作使用者

套件 **443/443 全绿**。

### v2.1.74（2026-09-19）

**配置区开头加了说明块（照 KiraAI 官方搜索插件的 `info` 写法）。**

KiraAI 的插件设置里，字段类型除了 `switch`/`string`/`integer` 之外还有
**`type: "info"`** —— 一段**纯说明**（不是输入框）。官方的联网搜索插件
开头就是这么一个 `info_intro`。现在我们也照同样的写法加了一块，放在**最前面**：

> **说明**
>
> **这里的一切都可以在侧边栏 WebUI 里配置，更快捷。** 打开 KiraAI 侧边栏 →
> 「全能浏览器」插件页：下面每一项都能在那儿改，**改完立刻生效、不用重启**；
> 那个页面是照着这份 schema 生成的，以后加了新选项它会自动出现。
> 同一页还能看连接状态、当前后端、扩展版本、命令计数，以及生成 / 复制接入令牌。
>
> **刚装完扩展时，建议把浏览器重新打开一次，再重新打开一次侧边栏 WebUI 页面**
> —— 这样最容易连上：扩展刚加载时它的后台进程可能还在休眠，
> 重开一次会让它立刻醒过来去认你当前这个端口。

**同一段文字**也会渲染在插件面板的配置区最上面（面板读的就是这份 schema）。

#### 顺带把"版面元素"和"配置项"分开

`info` / `section` 是**版面元素**，不是配置项 —— 混进来会出两个问题：

1. 面板会把它当输入框渲染（`info` 连 `default` 都没有）
2. "schema 里每一项都要被代码读取"这类守卫会把说明块报成**孤儿配置**

所以在三处统一滤掉：后端的 `_schema_fields()`、面板的 `loadConfig()`、
以及测试的 `harness.schema()`（一处过滤，所有守卫受益）。

#### 新增 S12

配置开头必须有说明块（照官方 `info` 写法），且**两个关键提示都在**：
"侧边栏 WebUI 可配" + "装完扩展后重开一次更容易连上"。

套件 **443/443 全绿**。

### v2.1.73（2026-09-19）

**侧边栏 WebUI：全部配置项都能在那儿改，改完立刻生效。**

#### 面板不再是"只读看板"

以前面板只能看状态、生成令牌 —— **配置得去翻配置文件**。现在它是一个**完整控制台**：

| | |
|---|---|
| **全部配置项** | 29 项，全在面板上改 |
| **界面怎么来的** | **从插件自己的 `schema.json` 生成** —— 插件加了新选项，界面**自动出现**，不用改前端 |
| **生效方式** | **改完立刻生效，不用重启 KiraAI** |
| 同一页还有 | 连接状态 / 当前后端 / 扩展版本 / 命令计数 / 令牌生成复制 |

**热更新怎么做的**：面板写进 `<插件数据目录>/webui_config.json`，
`_apply_config()` 把这些覆盖值**叠在框架配置之上**（覆盖优先），
保存时重跑一次 `_apply_config` —— 于是立刻生效 ✓

> `POST /config` 只接受 schema 里声明过的键 —— 免得前端拼错一个字就悄悄写入
> 一个没人读的字段（那种 bug 最难查）。

#### 设计

图标统一 **Lucide（内联 SVG）**，**界面全程无表情符号**。
配色取自插件图标本身：奶油 `#FBF6E9` / 青绿 `#1E8E88` `#2AA7A0` / 琥珀 `#C8873A` `#E0A44B`。
左侧栏分组导航 + 滚动高亮，暖色渐晕底纹，改动有"未保存"提示条，
保存成功有轻脉冲；`prefers-reduced-motion` 下自动关动效。

拆成三个文件：`web/index.html` + `web/style.css` + `web/app.js`
（框架把 `web/` 作为静态目录挂载，相对路径直接可用）。

#### 顺带修一个**真问题**

`refresh()` 原来把"取数"和"渲染"塞在同一个 `try` 里 ——
渲染里任何一个小错（比如某个 DOM 节点没找到）都会被 catch 当成**请求失败**，
界面显示"读取失败"，**把渲染问题误报成网络问题** ✗。
现在拆开了：取数失败才报"读取失败"，渲染异常不再冒充网络错误。

#### 新增 S10 / S11

- **S10** 侧边栏可配置全部项并热生效（读/写端点 + 叠覆盖 + schema 生成，缺一即红）
- **S11** README 的配置章节写明"侧边栏可配置、改完即生效"（找不到入口的话，再好的面板也白搭）
...
| 提的 | 原来 | 现在 |
|---|---|---|
| **剪贴板读写** | ✗ 没有 | ✅ `browser_interact(action="clipboard_get"/"clipboard_set")` |
| **多标签静音/固定** | ✗ 只支持单个 `tab_id` | ✅ 带 `query` 时**批量**作用于所有匹配标签 |
| **标签搜索** | ✗ 没有 | ✅ `browser_tabs(action="search", query=…)` |
| 读**选中的文字** | 🔴 **扩展一直有 `get_selection`，插件从没调用** | ✅ `browser_page(mode="selection")` |

**剪贴板的两个坑（都处理了）**：
- **不能用 `execJs`** —— 那条路走 `chrome.userScripts`，用户没在扩展详情页打开
  "允许用户使用脚本"就整个不可用 ✗。剪贴板是常用功能，不能绑在那个开关上。
  改用 `chrome.scripting.executeScript` 的 **ISOLATED world** —— 不需要那个开关 ✓
- **读**剪贴板要求页面**在前台聚焦**（浏览器隐私限制）。失败时会把原因说清楚，
  而不是笼统报错 ✓

**批量操作的做法**：插件侧先取标签列表 → 按关键词筛 → **逐个调用**
`mute_tab`/`pin_tab`/`close_tab`。扩展内部命令**不计次**，所以不用给扩展加
批量命令 ✓

#### 对账：扩展能执行的 40 个命令，插件全都够得着

| | |
|---|---|
| 扩展能执行的命令 | **40 个** |
| Python 协议里够不着的 | **0 个** ✓ |
| 暴露给模型的**工具** | **11 个 —— 没变** ✓（新能力全做成 action） |
| `browser_interact` 的动作 | 22 个 |
| `browser_tabs` 的动作 | 6 个（list/search/activate/close/mute/pin） |

#### 新增 S7：把"够不着"做成常驻检查

> 判据：协议里的每个命令，**必须能在 `main.py` 里找到 `_call("xxx")`**
> （真正内部的少数几个在白名单里）。

写这条判据踩了两次坑，都记在代码注释里：
1. **只查 `"xxx"` 字符串不行** —— 渲染分支里也会出现 `method == "get_selection"`，
   删掉调用、留下渲染照样绿
2. **"后端有同名方法"不能算够得着** —— `get_selection` 的后端方法一直有，
   `main.py` 从没调用过，正是这么漏掉的

反向验证：往协议里加一个没人调的假命令 → **S7 立刻点名它** ✓
...
#### bot 那两句话，一半是硬边界、一半是我们的疏忽

| bot 说的 | 真的吗 | 为什么 |
|---|---|---|
| `edge://bookmarks` 扩展没权限访问 | ✅ **真的** | **浏览器硬边界**：`<all_urls>` 也**不包含** `chrome://`/`edge://`，注入一律拒绝。**任何扩展都做不到**，这不是权限没开、也没法开 |
| `chrome.bookmarks` 没暴露 | ✅ **真的** | **因为我们没申请 `bookmarks` 权限** —— 这是**我们的选择**，不是浏览器限制 |

#### 修：该开的开，开不了的说清楚

**① 书签数据接口（现在直接可用）**

manifest 加 `bookmarks` 权限，新增能力 `bookmarks(query, max, folders_only)`，
接在 `browser_interact` 的 `action="bookmarks"` 上：

```
browser_interact(action="bookmarks")                   → 全部书签（含所在文件夹）
browser_interact(action="bookmarks", query="bilibili") → 按关键词搜（标题或网址）
browser_interact(action="bookmarks", folders_only=true) → 只要文件夹结构
```

> 用户要的是**书签本身**，不是那个页面 —— 数据接口才是正解。

**② 内部页的报错改成**可照做**的**

原来只说"不允许注入脚本，请先切换到普通网页" → 模型只能放弃、回用户一句
"扩展没权限访问"，看着像缺陷。现在会告诉它两条**能走通**的路：

- **截图对内部页照常有效**（走 `captureVisibleTab`，截的是可见区域，
  **不需要注入**）→ 配合 VLM 就能读到画面内容（书签列表、设置项…）
- 要**数据**不要页面 → 书签走 `action="bookmarks"`

**③ 渲染分支**

`bookmarks` 的结果原来是走**默认分支**被丢掉的（守卫 A1 抓的）——
现在有专门渲染：标题 + 网址 + 所在文件夹，按关键词回显。

#### 新增 S5 / S6

- **S5** 内部页报错必须可照做（提到硬边界 + 截图 + 书签接口）
- **S6** 书签能力可用（权限 + 实现 + action 三处都齐）

#### 顺带：`OpResult.fail` 支持 `data`

某个后端**天生做不到**时（无头没有书签库），失败结果也带上**同样的字段形状**
（空的），这样契约检查 F1"两个后端都返回渲染层要读的字段"才成立。
`ok` 仍然是 `False`，调用方拿到的还是 error、不会走到渲染。

套件 **437/437 全绿**（20 个方法的字段契约全部一致）。

### v2.1.68（2026-09-19）

**一步到位：写操作顺手把页面内容一起带回来（省掉一半工具调用）+ 无头默认安全 UA。**

#### ① 为什么必须合起来：框架每轮只给 **5 次**工具调用

用户日志里满屏：

```
WARNING [tool_use] Tool call limit exceeded: maximum 5 tool calls per turn,
        skipping tool 'browser_page'
```

那个 5 是框架的 `bot_config.agent.max_tool_calls_per_turn`（可配，默认 5）。
而"**点一下 → 再看一眼**"如果各占一次调用，5 次只够两轮半 —— bot 还没干完
就被限流。

**关键**：计的是 **LLM 发起的工具调用**，**扩展内部命令不计次**。
所以在插件内部顺手把页面取回来是**白赚的**。

**落点**：改在 `_call()` 里 —— 凡是 `for_write=True` 的操作，成功返回前
自动附上「现在的页面（标题+网址）」+「页面内容（截断 800 字）」。

- **一处改动覆盖所有写操作**（点击/输入/导航/滚动…），以后加新工具也自动生效
- `navigate` 同样受益（"导航→看"是最常见的两次）
- 新增开关 `return_page_after_write`（默认**开**），想省 token 可以关

#### ② 无头后端默认给**安全 UA**

| 后端 | 原来 | 现在 |
|---|---|---|
| 扩展桥（用户 Edge） | 用户真实 UA ✓ | 不变 ✓ |
| 无头（Playwright） | Playwright 默认 = `HeadlessChrome/...` ✗ B站会拦 | 真实桌面 UA（`Chrome/131 … Edg/131`）✓ |

UA 里**不含 `Headless`**，版本号尽量跟浏览器主版本对齐，平台按宿主系统推断。
用户自己填了 `user_agent` 就优先用他的。

> 表现上的差别：不带 Headless 的 UA 能正常开 B站/知乎这类站点；
> 带了的话常见症状是"页面能开但内容空 / 弹验证"，很难看出是 UA 的问题。

#### 新增检查 `one_step`（4 条）

- **S1** 写操作后一并返回页面（且必须挂在 `for_write` 上 —— 否则新加个写工具就漏了）
- **S2** 无头有安全 UA，且代码里说清了"为什么必须去掉 Headless"
- **S3** **实测**：真造一个无头后端，看 `user_agent` 到底是什么值
- **S4** 用户填了 `user_agent` 时优先用他的

套件 **435/435 全绿**。

### v2.1.67（2026-09-19）

**VLM 超时的真根因：图片**根本没压**就发出去了（8.6 MB → 0.88 MB）。**

#### 真根因（用户怀疑得对：30 秒超时本身就可疑）

`desc_img` 内部走 `image.to_data_url()` —— 那是**原样 base64**，
不缩放、不重编码；再配上它写死的 `detail: "high"`：

```
实测（1800×1200 截图）：原图 base64 = 8,653,804 字符（≈8.6 MB）
                        实发        =   882,919 字符（≈0.88 MB）
```

**不压就是 8.6 MB** —— 30 秒超时完全说得通。

框架自己在 `message_manager` 发消息前会调 `compress_image_element`，
但**我们直接调 `desc_img`，绕过了那一步**；
而且框架默认 `image_compression.enabled = **False**`（连它自己都默认不压）。

#### 修

| | 原来 | 现在 |
|---|---|---|
| VLM 前压缩 | **没有**（发原图） | **默认开**（`vlm_compress`，最长边 1280 / JPEG 质量 85） |
| `vlm_timeout` | 30 秒 | **60 秒**（压缩后正常几秒就回来，60 是余量） |

新增三个配置：`vlm_compress`、`vlm_compress_max_size`、`vlm_compress_quality`。

#### 顺带答一个功能问题：「只截图、不描述」有吗？

**一直有** —— `browser_screenshot` 的 `describe` 参数，传 `false` 就只截图不描述。
但原来那句说明只写"默认取插件配置（通常 true）"—— **bot 看不出什么时候该传 false**，
等于这个开关对它不存在。现在说明写清了：

> 页面上有很多文字/表单时，`browser_page` 的 DOM 文本通常比图更快更准 ——
> 那类场景传 false 可以省掉几秒到几十秒；但传了 false 你就**看不到画面本身**
> （布局、图片、验证码、渲染异常这类只有图里才有的信息）。拿不准就别传。

#### 新增 A15 / A16 / A17

- **A15** 发图给 VLM 前会先压缩（默认开）+ schema 里有这三项
- **A16** **实测**：真正发给模型的 data URL 必须明显短于原图
  > 夹具特意换成**真的 1800×1200 PNG** —— 假 PNG（只有魔数）PIL 打不开，
  > 压缩会静默返回 False，那条路径根本没被验到（看着绿）
- **A17** 「只截图不描述」可用，且说明写清了何时该关

套件 **431/431 全绿**（VLM 组 29 → 32 条）。

### v2.1.66（2026-09-19）

**修"又连不上了"的两个真凶 + 面板上把"打开一次这个页面"写在脸上。**

用户场景：KiraAI 装在 **127.0.0.1:5274**（不在候选端口里），
扩展 v1.3.1 装着也启用着，面板「扩展连接」一直**未连接**、
「扩展版本」是「—」（= 插件从没收到过握手）。

#### 真凶一：接入信息没变，就**什么都不做**

```js
if (!changed) { sendResponse({ ok: true, unchanged: true }); break; }   // ← 直接退出
```

`changed` 判的是"令牌/端口有没有变"。可**"没变"不等于"不用连"** ——
于是"令牌没变、但连接掉了"这一路**永远不会被救回来**：
用户明明正开着 KiraAI 面板、扩展明明就是断的，它却什么都不做。

现在：
- 已经连着 → 不动（保持"每 10 分钟轮询不打扰"的原意）
- **没连上** → 连它
- 用户手动点过「断开」→ 尊重他，不动

#### 真凶二：令牌作废后**永远不会回去重新配对**

插件侧的令牌与 KiraAI 的鉴权状态**绑定** —— KiraAI 重启、或轮换
`access_token` 之后旧令牌**当场作废**。而扩展只会在 `instances` 里
拿旧令牌一遍遍重连、**永远不会回去重新配对**。

现在：连续失败 3 次后，回**已知的那个地址**重新领一枚令牌
（`fetch(host:port + /pair)`）再连 —— **不重扫端口**，
所以 KiraAI 装在 5274 这种非常见端口也照样能自愈。

#### 顺带：面板把关键一步写在脸上

未连接时，面板「扩展连接」下面直接给下一步：

> 扩展没连上？**打开一次这个页面**就行 —— 扩展会从这个页面的地址
> 认出 KiraAI 在哪个端口（不用手填、也不用管端口是不是常见的）。
> 还不行的话：点一下浏览器上的扩展图标，看它自己报什么错。

> ⚠️ 这不是可选提示，而是**主接入路径**：扩展是靠页面自己的地址
> （host + port）认端口的（v1.3.0 起）。

#### 新增 O6 + 场景 6

- **O6** 面板未连接时必须告诉用户「打开一次这个页面」
- **场景 6** 服务器换令牌（模拟 KiraAI 重启）→ 扩展应回已知地址重新配对

套件 **427/427 全绿**。

### v2.1.65（2026-09-19）

**修掉"连上了但命令全部超时"的真 bug —— 结果发不回去。**
...
```
[tool_use] browser_tabs args: {}
[插件日志] [list_tabs] 结果不确定（可能已执行），不再换后端重试
tool_result: ⚠️ 命令 list_tabs 超时（20.0s），扩展没有响应
```


而 `browser_diag` 明说"扩展桥=**已连接**"—— 连着的，命令却不回。

真因在 `handleMessage`：

```js
case MSG.CMD:
  await runCommand(msg.id, msg.name, msg.params || {});   // ← 漏传 link
```

而 `sendRaw(obj, link)` 的第一行是：

```js
if (!link || !link.open) return false;    // link 是 undefined → 静默吞掉
```

**后果不是"发错人"，而是一条都发不出去**：命令执行完了，结果被 `sendRaw`
丢掉，插件那边干等 `op_timeout`（20 秒）报超时。
表面看像"扩展没反应 / 没连上"，实际连接好好的，只是**回话没人接**。

`shared.js` 顶上专门写了一段注释警告这类 bug（"每个 send* 都必须显式带上
目标连接"），偏偏这个调用点漏了。

#### 为什么以前的测试没抓到

`bridge_e2e` 用的是**手写的假扩展客户端** —— 它自己会回话，
**真 `background.js` 里的 dispatch 根本没有被跑过**。

`pair_e2e` 加载的是**真的** `background.js`，所以新增 **P15：从插件侧
发一条 `list_tabs` 进来，断言扩展真的把结果发回去了**。
反向验证：把 `link` 去掉 → P15 立刻报红。

#### 顺带：弹窗显示「允许用户使用脚本」的状态

有用户问"这个必须开吗、扩展能不能自己开"。答案是：

- **不是必须的** —— 只有「执行 JS」一个能力需要它；点击 / 输入 / 截图 /
  列标签页等等**全部照常**。
- **扩展打不开** —— 这是 Chrome/Edge 的刻意设计（`userScripts` 必须由用户
  在扩展详情页手动开启，防止扩展静默执行任意代码）。任何扩展都做不到。

所以弹窗现在会**检测并说明**：没开时显示一行提示 + 去哪儿开 +
...
**扩展现在能从你打开的 KiraAI 页面**自己问出**它在哪个端口 —— 不再只靠扫固定端口。**

#### 问题

自动发现只能在一小撮**硬编码的常见端口**（5267 / 8080 / 8000 …）上探测。
KiraAI 装在别的端口上时，扫多少次都扫不到 —— 扩展就一直是"未连接"。

硬扫 65535 个端口不现实；但有个一直没用上的入口：

> **用户总要打开 KiraAI 的页面才能用 bot —— 那个页面本身就带着真实的主机名和端口。**

#### 做法

`content.js` 本来就注入在所有页面上。现在它多做一件很小的事：

1. **只在"本机 / 私网"地址上做**（`127.0.0.1`、`localhost`、`::1`、
   `10.x`、`192.168.x`、`172.16-31.x`、`*.local`）——
   **公网站点一次请求都不会发**（O3 守这条）
2. 同源请求一次 `/api/plugin/headless_browser/pair`（同源，不会被 CORS 拦）
3. 返回令牌 → 把 `{host, port, token}` 递给后台，走的是**现成的 `page_pair` 那条路**

于是：**你打开 KiraAI 页面的那一刻，扩展就把端口学到了** ——
不用手填、不用扫端口，从别的机器访问（局域网 IP）也一样。

每个 origin 在每个标签页里只试一次（`sessionStorage`），不会反复发请求。

#### 新增检查 `origin_detect`（5 条）

用**最小的 DOM 桩加载真的 `content.js`**，验四类页面：

| | 页面 | 期望 |
|---|---|---|
| O1 | `127.0.0.1:9977`（非常见端口） | 问出 host/port/token 并递回后台 |
| O2 | `192.168.1.5:5268`（局域网） | 同样能问出来 |
| O3 | `example.com`（公网） | **一个请求都不发** |
| O4 | `127.0.0.1:3000`（不是 KiraAI） | 不误配对 |
| O5 | —— | 后台确实处理 `page_pair`（发出去有人接） |

#### 顺带

- 发现失败时把 **`triedPorts`**（试过哪些端口）带回状态，弹窗直接列出，
  并指路"真实端口在 `data/webui.json` 的 `port` 里"
- 保活路径在没有实例时**显式重试发现**（退避 0.5/1/2/5 分钟，封顶 5 分钟）。
  > 订正：我一度以为旧代码是"首次失败就永远不再试"的死局 —— **核对后是错的**，
  > 顶层 `bootstrap` 每次 Service Worker 唤醒都会重跑，重试一直都有。
  > 旧代码真正的毛病是**节奏**：每 30 秒把 15 个端口扫一遍，一直白扫。

套件 **423/423 全绿**。

### v2.1.63（2026-09-18）

**发现失败时把"试过的端口"摊给用户看 + 保活路径的发现节奏。**

#### 先说清楚**不是**什么（我一开始判断错了）

我一度以为 `ensureAlive()` 里那句 `if (!cfg.instances.length) return;` 是个
"首次发现失败就永远不会再试"的死局。**核对后是错的**：顶层 `bootstrap` 在
**每次 Service Worker 被唤醒时都会重跑**，里面也会 `connect()` → `discover()`，
所以重试一直都有。

旧代码真正的毛病是**节奏**：每次 30 秒闹钟都把 15 个候选端口扫一遍 ——
在"这台机器上根本没有 KiraAI"时就是**一直白扫**。

#### 改了什么

| | 原来 | 现在 |
|---|---|---|
| 没有实例时的重试 | 靠顶层 bootstrap 隐式重跑 | 保活路径里**显式**重试（带退避 0.5/1/2/5 分钟，封顶 5 分钟） |
| 发现失败的提示 | 只有一句"没找到" | 附带 **`triedPorts`**（试过哪些端口），弹窗直接列出 |
| 配对成功后 | —— | 清零退避，后台重试立刻恢复积极 |

退避封顶**故意压在 5 分钟**：太久的话，用户在浏览器开着之后才启动 KiraAI，
会干等半天才被接上。
...
> 已试过的端口（都不通）：5267, 8080, 8000, …
> KiraAI 的端口不在这里？在 KiraAI 数据目录的 `webui.json` 里能看到 `port`；
> 把它填到上面的「端口」框里即可。

**这是对"扩展装了却一直未连接"最直接的定位手段** —— 一眼就能看出是不是
"KiraAI 用了候选列表之外的端口"。

#### 新增 P14 / P14b / P14c

- **P14** 保活路径在没有实例时会按退避重试发现并连上
- **P14b** 发现失败时带回 `triedPorts`
- **P14c** 弹窗会把试过的端口显示出来，并指路 `webui.json`

套件 **418/418 全绿**。

### v2.1.62（2026-09-18）

**VLM 超时默认 10 秒 → 30 秒；没拿到描述时的内联文案收短。**

#### 超时

原来默认 **10 秒**，对视觉模型偏紧：一张大截图的编码 + 推理经常就要十几秒，
超时后描述被丢掉，用户看到的是"没拿到描述"。**默认改为 30 秒** ——
超时只是不附描述，截图本身照常返回、照常发给用户，所以放宽不增加风险。

四处一起改（**不能只改一处**）：`schema.json` 的 default、
`main.py` 的回落值、`vlm.describe_image` 的形参默认、以及 `timeout or` 的兜底。

#### 失败文案

原来失败时返回**六行排查说明**（"常见原因：没配视觉模型…模型配错了组…
调用超时…图片本身已保存…"）。这段会挤进**每一次**失败的返回里，既占
token 又干扰模型 —— 而真正要排查的人有更该去的地方。

现在内联只说一句：

> ℹ️ 这次没拿到图片描述（图已保存）。排查：browser_diag action="vlm"

细节全部留在 `browser_diag(action="vlm")` 里（它本来就写得很全：插件配置、
实际会用的模型、视觉能力判定、常见原因）。

#### 新增 A13a/A13b/A13c/A14

- **A13a** schema 里 `vlm_timeout` 默认是 30
- **A13b** 代码回落值也是 30，且与 schema 一致（只查 schema 的话，
  代码里偷偷留着 10 照样绿 —— 用户没在面板存过配置时走的就是代码回落）
- **A13c** `describe_image` 的形参默认也是 30
- **A14** 失败分支的内联文案必须简短且指向 `browser_diag`

> A14 的判据只查**截图工具那段**，不能全文查 —— "常见原因" 这个词在
> `_diag_vlm`（browser_diag 的详细诊断）里**本来就该有**，全文查会把它
> 一起算进来（首跑就是这么误报的）。

反向验证：把默认值改回 10、把长文案写回去 → A13a/A13b/A14 报红。

套件 **415/415 全绿**（VLM 组从 25 条增加到 29 条）。
...
**常驻检查 `vlm_describe`（29 项）——目的是防止它再次被弄丢**：
...
- **扩展后端截图"假成功"**：`screenshot(full_page=True)` / `selector=`
  ...
  元素截图。→ 明确返回失败，并告诉对方"想整页就显式切到无头后端"
  （2026-09-22 起插件不再自动换后端，见 v2.1.87）。
