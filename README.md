...
**安装引导改成当前流程 + 扩展弹窗显示版本号 + 补上"纯自动"端到端场景。**

#### 安装引导过时了

`setup_guide.steps_for()` 的最后一步还是**老的手动流程**：

> 扩展出现后，点它的图标，把「接入令牌」粘进去，点连接
> （令牌在 KiraAI 的插件面板里：「浏览器」→ 复制接入令牌）

但现在的实际行为是：**装完自己扫常见端口连上**；扫不到也能靠"打开面板即配对"，
**根本不用手填**。这段老文案还和扩展弹窗里写的"点「自动检测」即可，不用手填"
**自相矛盾**。

改成与现状一致的三步：装好不用填 → 打开面板即配对 → 自动检测或手填。

#### 扩展弹窗显示版本号

以前弹窗不显示版本，用户报"装了还是连不上"时，**没法确认他是没更新、还是新版真有 bug**。
现在标题旁显示 `v1.2.0`（扩展 manifest 同步 bump 1.1.0 → 1.2.0）。

#### 补上"纯自动"端到端场景（我之前的测试盲区）

`pair_e2e` 里所有场景都显式调了 `connect({manual:true})`，而 manual 会
**强制清掉 `userDisconnected` 闸门**再连 —— 等于把自动路径的前置条件全绕过去了，
**是假绿灯**。

新增场景 0：只加载扩展、**什么都不调**，让顶层 `bootstrap` 自己跑，
断言它自己完成"发现 → 配对 → 连上"。

#### 新增 P12/P13

- **P12** 安装引导与当前流程一致 —— **查渲染后的文本**，不是源码
  （`strip_comments_only` 是给 JS 写的，剥不掉 Python 的 `#` 注释，会命中自己的注释）
- **P13** 扩展弹窗显示版本号
...
**答"我的实例不是 5267，还能不能安装即用"——能，三条路；并修掉一条把人指错路的提示。**

#### 三条接入路径（都有测试证据）

| 你的情况 | 怎么接入 | 验证 |
|---|---|---|
| 端口在 15 个候选里（5267 打头） | 装完**自动发现**并连上，什么都不用填 | `pair_e2e` P1–P5 |
| 端口任意，从 `127.0.0.1` / `localhost` 打开插件面板 | **打开面板即配对** —— 面板页里 `location.port` 就是它的真实端口，多实例也不会认错 | P8/P9（实测非候选端口 7979） |
| 端口任意 + 从域名 / 局域网 IP 访问 | 弹窗里手动填「服务地址 / 端口 / 令牌」（令牌在面板点「复制」） | P11（指引与实际功能对齐） |

**候选端口探不到不是 bug**，是候选列表的边界 —— 所以另外两条路必须真的可用（P7 专门守着"这条确实探不到"这个前提，免得哪天有人把候选列表改了、P8 的前提悄悄失效）。

#### 修掉：提示文案把人指到**不存在的按钮**上

自动发现失败时原来写的是「可以在插件面板点**「复制接入配置」**，再粘贴到上面的输入框」——
但面板上**没有这个按钮**（它只做 `postMessage` 自动配对），弹窗里也没有粘贴解析。
用户照着找不到，等于指到死路。

改成两条**真实存在**的路：打开插件面板（自动配对）或手动填（令牌在面板「复制」）。

#### 新增 P7–P11

P7 非候选端口确实探不到 / P8 面板带入能配上非候选端口 / P9 插件侧 `bridge.connected` 为真 /
**P10 提示文案不引用不存在的 UI** / P11 指引与实际功能对齐。

> P10 的判据要**先剥注释**再查字符串 —— 注释里往往会提到"以前那句错话"，
...
**扩展零配置接入的静默丢弃 bug + 新增跨语言端到端检查。**

#### 找到的 bug：已有实例时，新配的实例会被静默丢掉

`applyPair()`（自动发现命中之后存实例的地方）**只写旧版单实例键**
（`kb_host` / `kb_port` / `kb_token`），**从不写 `kb_instances`**。
而 `getConfig()` 只在 `kb_instances` **不是数组**时才把旧键迁移成列表：

- **全新安装**：`kb_instances` 一直是 `undefined` → 每次都走迁移 → 能连上（侥幸）
- **已经配对过实例**：`kb_instances` 已是数组 → 迁移被跳过 → `applyPair` 写的东西
  **没有任何人读** → 新实例被静默丢弃

表现：点「自动检测」看着像成功了，实际什么都没变 —— 属于"装了却像没装"那一类。

→ `applyPair()` 改走 `upsertInstance()`（项目里其它地方用的都是这条路径），
旧键再同步写一份（回滚到老版本还能用；此时列表已是数组，migration 不会再覆盖它）。

#### 新增检查 `pair_e2e`（7 条，跨语言端到端）

以前 `bridge_e2e` 用的是**手写的假扩展客户端**，只验证桥协议本身；
"新装扩展能不能自己连上"这条路**从没被覆盖**，所以出过的两个真 bug
（SW 启动即崩、端口采信错）都是用户撞出来的。

这个检查是**真跨语言**：Python 起**真实的 `bridge.py`**（同一个端口同时答
HTTP `/pair` 和 WS 升级），Node 跑**扩展真实的 `background.js`**（stub 掉 chrome API）。

| 条目 | 盯什么 |
|---|---|
| P0 | 测试里写死的路径必须和 `protocol.js` 里的一致（否则测的不是真东西） |
| P1 | 全新安装（storage 全空）时扩展不报错 |
| P2 | 自动发现的实例真的存进了 `kb_instances` |
| P3 | 存的是**探测到的**端口（服务端故意报错端口也不会被带偏） |
| P4 | 状态显示已连接 |
| P5 | 插件侧 `bridge.connected` 变成 True（= "装了扩展"的判据） |
| P6 | 已有实例时「自动检测」会把新实例**加进列表**（不是静默丢弃） |

**反向验证**：把 `applyPair` 写回旧版 → P2/P3/P6 立刻报红，
其中 P6 明确显示"探测到 5267，列表里只有旧的 9999"。

> 写这个检查时踩到两个坑，都记在代码注释里：
> ① `subprocess.run` 是阻塞的，会把事件循环卡住 —— 服务器跑在同一个循环上，
>    于是扩展的所有探测都超时。看起来像扩展的 bug，其实是测试把服务器憋死了。
> ② `bridge.connected` 只能在扩展**还连着的时候**采样 —— Node 客户端跑完就
>    `process.exit`，跑完再读永远是 False。
