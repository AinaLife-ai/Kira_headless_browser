...
def steps_for(browser: str, ext_path: str) -> List[str]:
    ...
    common += [
        "**装好后不用填任何东西** —— 扩展会自动找到本机的 KiraAI 并连上"
        "（默认 5267 及常见端口）。",
        "自动找不到时（KiraAI 用了不常见的端口）：从 127.0.0.1 或 localhost "
        "打开一次 KiraAI 的「全能浏览器」面板 —— **打开就会自动配对**。",
        "还是不行：点扩展图标 →「自动检测」；或在弹窗里手动填"
        "「服务地址 / 端口 / 令牌」（令牌在面板上点「复制」拿）。",
    ]
    if b == "edge":
